...
class WSMServiceTest(TestCase):
    def setUp(self):
        ...
        for ch in range(13, 54):
            TVChannelStatus.objects.create(facility=self.facility, channel_number=ch, is_available=False)

    def test_wsm_csv_generation_basic(self):
        """基本的なCSV生成と、選択されたチャンネルの反映をテスト"""
        # 13CHと53CHを「空き」にし、13CHを「選択」する
        TVChannelStatus.objects.filter(facility=self.facility, channel_number__in=[13, 53]).update(is_available=True)
    ...
    def test_wsm_csv_continuous_channels(self):
        ...
        # 13CH, 14CH を「空き」にする
        TVChannelStatus.objects.filter(facility=self.facility, channel_number__in=[13, 14]).update(is_available=True)
