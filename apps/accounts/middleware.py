...
5. ユーザーが確定したら Django セッションにログイン状態を記録

【セキュリティ設計】
- JWT が無効・期限切れの場合はスキップ（既存認証にフォールバック）
- JWKS はライブラリによりメモリキャッシュ（再起動まで保持）
- 既存の LINE WORKS SSO ログインフロー（auth/lineworks/）は削除せずに残す
  → 移行期間中の安全弁として機能する

【設定項目 (settings.py)】
- PORTAL_JWKS_URL: JWKSエンドポイントURL（例: https://portal.shin-on1981.com/api/jwks/）
