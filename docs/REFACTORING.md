...
- **モジュール化**: 機能ごとに JS ファイルを分割（`api`, `keep-list`, `form-storage`, `pdf-preview`, `adjustment-form`, `app-state`, `form-service`, `validation-service`, `ui-renderers`, `constants`）。
...
フロントエンド刷新の最終仕上げとして、以下のチェックリストを順次実施します。
